...
uint32_t TanhCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kTanh);
  auto data_type = ctx.Input(0)->GetDataType();
  switch (data_type) {
    Tanh_COMPUTE_CASE(DT_COMPLEX64, std::complex<float>, ctx)
    Tanh_COMPUTE_CASE(DT_COMPLEX128, std::complex<double>, ctx)
    Tanh_COMPUTE_CASE(DT_FLOAT, float, ctx)
    Tanh_COMPUTE_CASE(DT_FLOAT16, Eigen::half, ctx)
    Tanh_COMPUTE_CASE(DT_DOUBLE, double, ctx) default
        : KERNEL_LOG_ERROR("Tanh kernel data type [%s] not support.",
