...
uint32_t CholeskyGradCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  if ((shape0->GetDimSize(dims-1) != shape2->GetDimSize(dims-1)) ||
      (shape0->GetDimSize(dims-Num2) != shape2->GetDimSize(dims-Num2))) {
  ...
  if (data_type_0 != DT_FLOAT && data_type_0 != DT_DOUBLE) {
    KERNEL_LOG_ERROR("CholeskyGrad kernel data type [%u] not support.",
                     data_type_0);
    return KERNEL_STATUS_PARAM_INVALID;
  }
  ...
  if (data_num >= kParallelDataNum) {
    ...
    auto sharder_cholesky_grad = [&](int64_t start, int64_t end) {
      for (int64_t i = start; i < end; i++) {
        ComputeMatrix(lptr + i * mat_size,
                      gradptr + i * mat_size,
                      outputptr + i * mat_size,
                      n);
      }
    };
    ...
    KERNEL_HANDLE_ERROR(
        CpuKernelUtils::ParallelFor(ctx, batch, batch / max_core_num,
                                    sharder_cholesky_grad),
...
void CholeskyGradCpuKernel::ComputeMatrix(T *lptr, T *gradptr, T *outputptr, int64_t n){
    ...
    output_matrix = (NumHalf * (output_matrix + output_matrix.transpose())).eval();
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
  ...
  template <typename T>
  void CholeskyGradCpuKernel::CholeskyGradUnblocked(const Eigen::Ref<const Eigen::Matrix<T,
                                                    Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>& l_block,
                                                    Eigen::Ref<Eigen::Matrix<T, Eigen::Dynamic,
                                                    Eigen::Dynamic, Eigen::RowMajor>> grad_block) {
    ...
    for (int64_t k = kMatrixSize - 1; k >= 0; k--) {
      ...
      if (d == 0) {
        KERNEL_LOG_ERROR("d should not be 0.");
      }
